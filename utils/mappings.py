...
def build_owner_mapping(sf_source, sf_target, ownerIds):
    ...
    if source_group_ids:
        ...
        soql_source = f"""
            ...
            AND Type = 'Queue'
        """
        source_groups = safe_query(sf_source, soql_source)["records"]
        ...
        if source_groups:
            ...
            if dev_names:
                soql_target = f"""
                    ...
                    AND Type = 'Queue'
                """
                target_groups = safe_query(sf_target, soql_target)["records"]
    ...
    for i in range(0, len(id_list), batch_size):
        ...
        if user_ids:
            ...
            soql_user = f"""
                ...
                WHERE Card_Legacy_Id__c IN ({ids_str})
            """
            user_results = safe_query(sf_target, soql_user)["records"]
...
def fetch_service_appointment_ids(sf, sa_ids=None, batch_size=500):
    ...
    for i in range(0, len(sa_ids), batch_size):
        ...
        try:
            ...
            results_sa1 = safe_query(sf, query_sa1)
        ...
        try:
            ...
            results_sa2 = safe_query(sf, query_sa2)
...
def get_record_type_id(sf_connection, object_name, record_type_name="Standard"):
    """
    Get RecordType ID dynamically instead of hardcoding.
    Drop-in replacement for hardcoded RecordType IDs.
    """
    try:
        query = f"""
        ...
        if result['records']:
            ...
            # Try to find any active RecordType as fallback
            fallback_query = f"""
                SELECT Id FROM RecordType 
                WHERE SObjectType = '{object_name}' 
                AND IsActive = true
                LIMIT 1
            """
            fallback_result = sf_connection.query(fallback_query)
            if fallback_result['records']:
                print(f"Warning: Using fallback RecordType for {object_name}")
                return fallback_result['records'][0]['Id']
            else:
                raise ValueError(f"No RecordType found for {object_name}")
                
    except Exception as e:
        print(f"Error resolving RecordType for {object_name}: {e}")
        raise
